...
Test.@testset "git tests" begin
    ...
    Test.@test(
            RemoveLFS.Utils.command_ran_successfully!!(
                    git_version_cmd
                    )
            )
...
#             )
    ...
    Test.@test(
            "Hello There" ==
                    RemoveLFS.Utils.retry_function_until_success(
                            () -> f_1()
                            )
            )
...
#             )
    ...
    Test.@test(
        typeof(RemoveLFS.Utils.git_version()) <: VersionNumber
        )
    Test.@test(
        typeof(RemoveLFS.Utils.get_all_branches_local()) <:
            Vector{String}
        )
    Test.@test(
        typeof(RemoveLFS.Utils.get_all_branches_local_and_remote()) <:
            Vector{String}
        )
    Test.@test(
        typeof(RemoveLFS.Utils.get_current_branch()) <: String )
    Test.@test(
        RemoveLFS.Utils.branch_exists("branch1") )
    Test.@test(
        !RemoveLFS.Utils.branch_exists("non-existent-branch") )
    Test.@test(
        !RemoveLFS.Utils.branch_exists("non-existent-but-create-me") )
    Test.@test(
        typeof(RemoveLFS.Utils.checkout_branch!("branch1")) <: Nothing )
#     Test.@test_throws(
#         ErrorException,
...
#         )
    Test.@test_warn(
        "",
        RemoveLFS.Utils.checkout_branch!(
            "non-existent-branch";
            error_on_failure=false,
            ),
        )
    ...
    Test.@test(
        typeof(RemoveLFS.Utils.git_push_upstream_all!()) <: Nothing
        )
    run(`git checkout master`)
    ...
    branches_to_snapshot::Vector{String} =
        RemoveLFS.Utils.make_list_of_branches_to_snapshot(
            ;
